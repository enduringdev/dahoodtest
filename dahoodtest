...
print("=")
  ...
  local MainColor = Color3.fromRGB(255, 151, 102)
  ...
  local AntiAimViewer_Color = Color3.fromRGB(255, 151, 102)
...
local ItemTable = {
  ...
  local Settings = {
    ...
    ['Visuals'] = {
        ...
        ['Bullet_Trails'] = {
            ...
            Segments = 10,
            LightEmission = 10,
            Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(99, 67, 230)), ColorSequenceKeypoint.new(1, Color3.fromRGB(99, 67, 230))}),
            Speed = 3,
    ...
    ['Misc'] = {
        ...
        ['TrashTalk'] = {
            ...
            ['Words'] = {
                enduring = {"do you have $$$$endurscripts!!!", "MoveDirection isn't a resolver", "Skid🤣", "WHERE BRO AIMING AT!?", "Lost 2 enduring", "omg so bad", "imagine dying to me", "whats the point of playing with that aim🙏😂", "WOWZIES", "OH MY DAYS", "enduring'd", "1d luh bro", "WOW U SUCK", "Im right here😂", "couldnt be me", "just broke ur ankles", "UR ANKLES? GONE?", "destroyed", "LOL DESTROYED", "LOL"},
            },
        },
    };
  ...
  getgenv().MethodBeingUsed = Settings.Misc.TrashTalk.Words.enduring
...
local function AutoBuy(Obj, Ammo, Amount)
  ...
  local Menu = loadstring(req({Url = "https://pastebin.com/raw/55iKqxgB", Method = "Get"}).Body)(); -- WYVJcRAt
    ...
    local function MenuNameUpdate()
        while (task.wait()) do
            local Name, PlaceHolder = 'enduring', ''
            for i = 1, #Name do --try
                local Character = string.sub(Name, i, i)
                PlaceHolder = PlaceHolder .. Character
                Menu:SetTitle(PlaceHolder .. '<font color="#' .. tostring(Menu.Accent:ToHex()) .. '">.do</font>')
                task.wait(.25)
            end
        end
    end
    ...
    Menu.Watermark()
    Menu.Watermark:Update('enduring<font color="#' .. tostring(Menu.Accent:ToHex()) .. '">.do</font>') -- please fix this and we can add a variable for a color.
  
    local Window = Menu:SetTitle('enduring') do
        ...
        local MiscTab = Menu.Tab("Misc") do
            ...
            local MTSection5 = Menu.Container("Misc", "Teleport", "Right") do
                Menu.ComboBox("Misc", "Teleport", "Place :", "Choose", {"Military","Revolver","High Medium Armor","Food","Gas Station","School","Ufo","Bank","Gym Top","Casino","Uphill","PlayGround","Flank"}, function(State)
                    local CFrameValues = {
        ...
        local VisualsTab = Menu.Tab("Visuals") do
            ...
            local TLHightlightSection = Menu.Container("Visuals", "ESP", "Left") do
                ...
                Menu.ColorPicker("Visuals", "ESP", "HigherHealthColor", Color3.fromRGB(255, 151, 102), 0, function(a)
                ...
                Menu.ColorPicker("Visuals", "ESP", "LowerArmorColor", Color3.fromRGB(255, 151, 102), 0, function(a)
                    getgenv().esp.BarLayout.armor.color_empty = a
                end)
                Menu.ColorPicker("Visuals", "ESP", "HigherArmorColor", Color3.fromRGB(255, 151, 102), 0, function(a)
                ...
                Menu.ColorPicker("Visuals", "ESP", "ChamsInnerColor", Color3.fromRGB(255, 151, 102), 0, function(a)
...
local index; index = hookmetamethod(game, '__index', function(Obj, Prop)
  ...
  function TeleportPlace(PlaceId, JobId)
    if JobId == game.JobId and #Utility:GetPlayers() <= 1 then
        Client:Kick("enduring: Current Server has 0 players, Rejoining.")
        --
        Wait()
        --
              ...
              if Settings.Visuals.Hit_Detection.Enabled then
                  ...
                  if (Target ~= nil) then
                      ...
                      if Settings.Visuals.Hit_Detection.Notify then
                          Menu.Notify('enduring<font color="#' .. tostring(MainColor:ToHex()) .. '">.do</font>  >  ' .. '+1 Hit | ' .. tostring(HitPart) .. ' | Target : ' .. Target.DisplayName .. '', 1.5)
                      end
